...
ans_homo = data.frame(session_id = double(), max_homo = double(), stringsAsFactors = FALSE)

for (i in 1:dim(df)[1]) {
  
  df_ans_homo = read.csv(paste0(df$session_id[i], '.csv'), header = TRUE, sep = ",", stringsAsFactors = FALSE)
  max_homo = max(table(df_ans_homo$Answer))
  ans_homo[i, 1] = df_ans_homo[1, 1]
  ans_homo[i, 2] = max_homo
}

write.table(ans_homo, file = paste0('ans_homo', '.csv'), sep = ",", col.names = TRUE,
            row.names = FALSE, qmethod = "double")
