...
#############################################################################
library(dplyr)
# This is to calculate index of Signal Detection Theory
# Create data frame

sdt= data.frame(session_id = double(), n_hit = double(), n_fa = double(), n_miss = double(),
                n_cr = double(), sum = double(), stringsAsFactors = FALSE)
...
for (i in 1:dim(df)[1]) {
  ...
  for (j in 1:dim(df_index)[1]) {
    if (df_index[j, 3] == 0 & df_index[j, 5] == 'Walker') {
      h = h + 1
    }
  }
  ...
  f = 0
  for (j in 1:dim(df_index)[1]) {
    if (df_index[j, 3] == 1 & df_index[j, 5] == 'Walker') {
      f = f + 1
    }
  }
  ...
  m = 0
  for (j in 1:dim(df_index)[1]) {
    if (df_index[j, 3] == 0 & df_index[j, 5] == 'No Walker') {
      m = m + 1
    }
  }
  ...
  c = 0
  for (j in 1:dim(df_index)[1]) {
    if (df_index[j, 3] == 1 & df_index[j, 5] == 'No Walker') {
      c = c + 1
    }
  }
  sum = h + f + m + c
...
# sdt = na.omit(sdt)
write.table(sdt, file = paste0('sdt', '.csv'), sep = ",", col.names = TRUE,
            row.names = FALSE, qmethod = "double")
...
df_sdt_stats = cbind(df, df_sdt_stats[2:11])
write.table(df_sdt_stats, file = "sdt_stats.csv", sep = ",", col.names = TRUE,
            row.names = FALSE, qmethod = "double")
